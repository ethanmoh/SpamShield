...
class SixthViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
    ...
    private var models = [AllowNums]()
    private var models2 = [BlockNums]()
    ...
    @objc private func didTapAdd() {
        let alert = UIAlertController(title: "Allow Number", message: "Add new number", preferredStyle: .alert)
        ...
        alert.addAction(UIAlertAction(title: "Submit", style: .default, handler: { [weak self] _ in
            ...
            if let formatted = self?.formatPhoneNumber(text) {
                self?.createItem(number: formatted)
            } else {
                self?.createItem(number: text)
            }
        }))
    ...
    func getAllItems() {
        do {
            models = try context.fetch(AllowNums.fetchRequest())
            models2 = try context.fetch(BlockNums.fetchRequest())
    ...
    func createItem(number: String) {
        ...
        let newItem = AllowNums(context: context)
    ...
    func deleteItem(item: AllowNums){
        context.delete(item)
        do{
            try context.save()
            getAllItems()
    ...
    func updateItem(item: AllowNums, newNum: String) {
